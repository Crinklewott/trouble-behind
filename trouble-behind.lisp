...
(defun describe-path (edge)
  ...
  (let ((place (caddr edge))
	(direction (cadr edge)))
    (let ((a/an (if (begins-with-vowel (prin1-to-string place)) 'an 'a)))
  (list 'there 'is a/an place 'to 'the direction 'of 'here.))))
...
(defun can-see (item)
  ...
  (eq *player-location* (caar (get-item-details item))))
